"""Shared strategy and trade-candidate contracts for vNext.

Strategies interpret immutable market facts for their own horizon. This module
does not detect structure, choose direction, size orders, or call a broker.
"""
...
STRATEGY_STATES = (
    "INACTIVE", "ELIGIBLE", "WATCHING", "ARMED", "TRIGGERED",
    "CANDIDATE_CREATED", "LLM_APPROVED", "RISK_APPROVED", "ORDER_PENDING",
    "POSITION_ACTIVE", "CLOSED", "INVALIDATED", "EXPIRED", "REJECTED", "CANCELLED",
)
TERMINAL_STATES = frozenset({"CLOSED", "INVALIDATED", "EXPIRED", "REJECTED", "CANCELLED"})
ALLOWED_TRANSITIONS = {
    "INACTIVE": {"ELIGIBLE"}, "ELIGIBLE": {"WATCHING", "INACTIVE"},
    "WATCHING": {"ARMED", "INACTIVE", "EXPIRED"}, "ARMED": {"TRIGGERED", "EXPIRED", "INVALIDATED"},
    "TRIGGERED": {"CANDIDATE_CREATED", "REJECTED", "INVALIDATED"},
    "CANDIDATE_CREATED": {"LLM_APPROVED", "REJECTED", "EXPIRED"},
    "LLM_APPROVED": {"RISK_APPROVED", "REJECTED"}, "RISK_APPROVED": {"ORDER_PENDING", "REJECTED"},
...
class TradeCandidate:
    ...
    def __post_init__(self) -> None:
        if self.direction not in {"buy", "sell"}:
            raise ValueError("candidate direction must be buy or sell; wait is not a candidate")
        if self.pair != self.strategy.pair:
            raise ValueError("candidate pair must match strategy pair")
        if not all((self.candidate_id, self.entry_trigger, self.zone_id, self.invalidation, self.state_hash)):
            raise ValueError("candidate requires trigger, zone, invalidation, and state hash")
        if not self.target_zone_ids:
            raise ValueError("candidate requires at least one target zone")
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            "candidate_id": self.candidate_id, "pair": self.pair,
