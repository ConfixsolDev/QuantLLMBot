...
class TimescaleIntelligenceStore:
    ...
    def append_events(self, events: list[dict]) -> int:
        ...
        for event in events:
            ...
            event_params.append((event_time, event["event_id"], event["symbol"], event["timeframe"],
                                 event["event_type"], event.get("evidence_id"), canonical(payload),
                                 digest(payload), now))
            outbox_params.append((event["event_id"], event_time, now))
        with self.pool.connection() as connection, connection.cursor() as cur:
            cur.executemany(event_sql, event_params)
            cur.executemany(
                "INSERT INTO graph_projection_outbox(event_id,event_time_utc,enqueued_at_utc) VALUES(%s,%s,%s) ON CONFLICT DO NOTHING",
                outbox_params,
            )
        # Exact inserted count is not required for idempotent migration; the
        # acceptance gate checks durable target counts after the source scan.
        return len(events)
