...
def test_approved_candidate_passes_risk_then_broker_adapter():
    ...
    result = engine.evaluate(
        ...
        risk_inputs={"account_equity": 10000, "max_cash_loss": 100, "entry": 10, "stop": 9,
                     "target": 12, "direction": "buy", "point_value": 1, "tick_size": 1,
                     "tick_value": 1, "volume_min": .1, "volume_max": 100, "volume_step": .1},
    )
    assert result.arbitration.decision == "APPROVE"
    assert result.risk.approved
    order = build_order_intent(result.candidate, result.risk)
    assert engine.submit(result, order=order)["state"] == "SUBMITTED"
    assert broker.orders == [order]


def test_engine_rejects_order_that_differs_from_approved_risk():
    broker = FakeBroker()
    engine = VNextEngine(pair="XAUUSD", frontier=TimeFrontier.from_value("2026-01-01T00:05:00Z"), broker=broker)
    state = engine.compose_state(bars())
    result = engine.evaluate(
        state, spec(), response={"decision": "APPROVE", "candidate_id": "c1", "state_hash": state.state_hash},
        candidate_inputs={"candidate_id": "c1", "direction": "buy", "zone_id": "z1", "invalidation": "z1-low", "target_zone_ids": ("z2",)},
        risk_inputs={"account_equity": 10000, "max_cash_loss": 100, "entry": 10, "stop": 9,
                     "target": 12, "direction": "buy", "point_value": 1, "tick_size": 1,
                     "tick_value": 1, "volume_min": .1, "volume_max": 100, "volume_step": .1})
    ...
    with pytest.raises(RuntimeError, match="differs"):
        engine.submit(result, order=order)
...
def test_engine_persists_candidate_to_risk_lifecycle_before_submission():
    ledger = InMemoryLedger()
    engine = VNextEngine(pair="XAUUSD", frontier=TimeFrontier.from_value("2026-01-01T00:05:00Z"), ledger=ledger,
                         broker=FakeBroker())
    state = engine.compose_state(bars())
    result = engine.evaluate(
        ...
        risk_inputs={"account_equity": 10000, "max_cash_loss": 100, "entry": 10, "stop": 9,
                     "target": 12, "direction": "buy", "point_value": 1, "tick_size": 1,
                     "tick_value": 1, "volume_min": .1, "volume_max": 100, "volume_step": .1})
    engine.submit(result, order=build_order_intent(result.candidate, result.risk))
    states = [event.payload["state"] for event in ledger.read(pair="XAUUSD")
              if event.event_type == "STRATEGY_LIFECYCLE"]
    assert set(states) == {"CANDIDATE_CREATED", "LLM_APPROVED", "RISK_APPROVED", "ORDER_PENDING"}
