"""Daily trade review — single file, step by step.

Reads one day's proposals, executions, and management-review snapshots from
the deployed QwenTradeReviewer logs, reconstructs every filled trade end to
end, and writes a ready-to-finish daily review markdown file matching
model_training\\DAILY_REVIEW_TEMPLATE.md. This replaces the ad-hoc grep/
one-liner approach used for the 2026-08-06 review with one reusable script.

Usage (run with the same Python the trading system uses):
    python daily_trade_review.py
    python daily_trade_review.py --date 2026-08-05
    python daily_trade_review.py --log-dir "C:\\Users\\HP\\AppData\\Local\\QwenTradeReviewer\\logs" --output-dir "D:\\QuantLLMBot\\model_training\\reviews"

The flow, step by step (see main() at the bottom — read that first):
    1. Resolve which date to review and where the logs/output live.
    2. Load that day's proposals (the new per-day file if the logging patch
       has been applied, otherwise filtered out of the legacy monolithic
       file — this script works either way).
    3. Load that day's execution events the same way.
    4. Load that day's management-review snapshots (reviews-YYYY-MM-DD.jsonl
       already writes one file per day, so no fallback is needed there).
    5. Index execution events by proposal_id, and count how many times each
       resulting position actually appeared in a management-review snapshot
       before it closed (this is what caught, on 2026-08-06, that 5 of 12
       filled trades closed before Qwen's management ever got to look at
       them — worth checking every day, not just once).
    6. Reconstruct one narrative record per filled trade: entry rationale,
       fill, close reason, P&L, peak/drawdown, holding time.
    7. Compute the day's aggregate stats.
    8. Render the markdown report and write it to
       model_training\\reviews\\YYYY-MM-DD_review.md, with sections 1-2
       filled in and sections 3-5 left as prompts for the human lesson-
       review pass described in DAILY_REVIEW_PROCESS.md.
"""
...
DEFAULT_OUTPUT_DIR = Path(__file__).resolve().parent / "reviews"
...
# ---------------------------------------------------------------------------
# Step 8: render the markdown report, matching DAILY_REVIEW_TEMPLATE.md.
# ---------------------------------------------------------------------------
SUMMARY_LABELS = [
...
def render_report(target_date, summary: dict, trades: list) -> str:
    ...
    lines += [
        ...
        "*(fill in by hand — see DAILY_REVIEW_PROCESS.md section 3)*", "",
        "## 4. Candidate engineering findings", "",
        "*(fill in by hand — see DAILY_REVIEW_PROCESS.md section 4)*", "",
        "## 5. Ledger updates made today", "",
        "*(fill in by hand — see LESSON_LEDGER.md)*", "",
    ]
    return "\n".join(lines)
...
def main():
    ...
    output_path = output_dir / f"{target_date:%Y-%m-%d}_review.md"
