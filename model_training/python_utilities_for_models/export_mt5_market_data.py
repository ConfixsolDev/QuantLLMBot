#!/usr/bin/env python3
"""Download MT5 historical data for training (CSV + side-by-side JSONL).

Each chunk writes `{name}.csv` and `{name}.jsonl` with the same rows.
Also writes `examples/*.example.jsonl` for each data type.

Canonical process doc: model_training/CURRICULUM_AND_DATA_PREP.md

Downloads:
  - Tick prints (daily chunks)
  - M1 / H1 from MT5; M5/M15/M30 from M1; H4(NY) from H1

Requires MetaTrader 5 terminal logged in (same as live app).

Example:
  cd E:\\QuantLLMBot\\model_training
  ..\\apps\\qwen_trade_software\\backend\\.venv\\Scripts\\python.exe scripts\\export_mt5_market_data.py --symbol XAUUSDr
"""
    ...
    *,
    ...
    for chunk_start, chunk_end in day_range(start, end):
        ...
        if not rows:
            ...
            continue
        count = write_csv_and_jsonl(path, TICK_COLUMNS, rows, kind="tick")
        stats.rows += count
        stats.files += 1
        append_log(
            log_path,
            {
                ...
                "rows": count,
                "enriched": "volume_spread",
                "from": iso_utc(chunk_start),
                "to": iso_utc(chunk_end),
            },
        )
        print(f"  ticks {day_key(chunk_start)}: {count:,} rows (+jsonl volume)")
    return stats
    ...
    *,
    ...
    for chunk_start, chunk_end in month_range(start, end):
        ...
        if not rows:
            ...
            continue
        count = write_csv_and_jsonl(path, CANDLE_COLUMNS, rows)
        stats.rows += count
        stats.files += 1
        append_log(
            log_path,
        ...
        print(f"  M1 {month_key(chunk_start)}: {count:,} bars (+jsonl levels/volume)")
    return stats
...
def export_rates(
    ...
    *,
    ...
    for chunk_start, chunk_end in month_range(start, end):
        ...
        print(f"  {timeframe} {month_key(chunk_start)}: {count:,} bars (+jsonl)")
    return stats
...
def derive_from_m1(symbol: str, out_dir: Path, log_path: Path, *, force: bool) -> ExportStats:
    ...
    for target in ("M5", "M15", "M30"):
        ...
        for m1_path in sorted(m1_dir.glob("*_M1.csv")):
            ...
            count = write_csv_and_jsonl(out_path, CANDLE_COLUMNS, derived)
            stats.rows += count
            stats.files += 1
            append_log(
                log_path,
                {
                    ...
                    "path": str(out_path),
                    "jsonl": str(out_path.with_suffix(".jsonl")),
                    "rows": count,
                },
            )
            print(f"  derived {target} {month}: {count:,} bars (+jsonl)")
    return stats
...
def derive_h4_ny(symbol: str, out_dir: Path, log_path: Path, *, force: bool) -> ExportStats:
    ...
    for h1_path in sorted(h1_dir.glob("*_H1.csv")):
        ...
        print(f"  derived H4(NY) {month}: {count:,} bars (+jsonl)")
    return stats
...
def main() -> None:
    ...
    examples_dir = write_type_examples(out_dir)
    print(f"Schema examples: {examples_dir}")
    append_log(
        log_path,
        {
            "event": "type_examples_written",
            "path": str(examples_dir),
            "types": ["tick", "M1", "M5", "M15", "M30", "H1", "H4"],
            "canonical_doc": "model_training/CURRICULUM_AND_DATA_PREP.md",
        },
    )
